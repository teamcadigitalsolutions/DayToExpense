...
export default function SmartHubPage() {
  ...
  // ---------------------------------------------------------------------------
  // TAB 1: RECEIPTS & WARRANTY LOCKER STATE (NO DUMMY DATA)
  // ---------------------------------------------------------------------------
  ...
  useEffect(() => {
    if (wsId) localStorage.setItem(`smart_receipts_${wsId}`, JSON.stringify(receipts));
  }, [receipts, wsId]);
  ...
  // ---------------------------------------------------------------------------
  // TAB 2: GROUP EXPENSE SPLITTER STATE (NO DUMMY DATA)
  // ---------------------------------------------------------------------------
  ...
  useEffect(() => {
    if (wsId) localStorage.setItem(`smart_groups_${wsId}`, JSON.stringify(groups));
  }, [groups, wsId]);
  ...
  // ---------------------------------------------------------------------------
  // TAB 3: DEBT PAYOFF PLANNER STATE (NO DUMMY DATA)
  // ---------------------------------------------------------------------------
  ...
  useEffect(() => {
    if (wsId) localStorage.setItem(`smart_debts_${wsId}`, JSON.stringify(debts));
  }, [debts, wsId]);
  ...
  const [liquidSavings, setLiquidSavings] = useState<number>(() => {
    const s = localStorage.getItem(`runway_savings_${wsId}`);
    return s ? Number(s) : 0;
  });
  const [monthlyEssentialExpense, setMonthlyEssentialExpense] = useState<number>(() => {
    const e = localStorage.getItem(`runway_expenses_${wsId}`);
    return e ? Number(e) : 0;
  });
  ...
  // ---------------------------------------------------------------------------
  // TAB 5: SUBSCRIPTION AUDIT STATE (NO DUMMY DATA)
  // ---------------------------------------------------------------------------
  ...
  useEffect(() => {
    if (wsId) localStorage.setItem(`smart_subscriptions_${wsId}`, JSON.stringify(subscriptionsList));
  ...
  useEffect(() => {
    if (wsId) localStorage.setItem(`smart_tax_${wsId}`, JSON.stringify(taxData));
  }, [taxData, wsId]);
