  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-blue-600"><RefreshCw size={18}/><span className="font-medium text-gray-800">Account Transfers</span></div>
        <button onClick={() => setShowForm(s=>!s)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700"><Plus size={16}/> New Transfer</button>
      </div>

      {showForm && (
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-gray-800 mb-4">Create Transfer</h3>
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 px-3 py-2 rounded mb-4">Transfers do NOT appear as Income or Expense — they only move money between your accounts.</p>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs font-medium text-gray-600 mb-1 block">From Account</label>
            ...
            <div><label className="text-xs font-medium text-gray-600 mb-1 block">To Account</label>
            ...
            <div><label className="text-xs font-medium text-gray-600 mb-1 block">Amount (₹)</label>
              <input type="number" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="0.00" min="0" /></div>
            <div><label className="text-xs font-medium text-gray-600 mb-1 block">Date</label>
              <input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
          </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {transfers.length === 0 ? (
              ...
              <tr key={t.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-700">{t.from_account_name ?? t.from_account_id?.slice(0,8)}</td>
                <td className="px-4 py-3 text-gray-700">{t.to_account_name ?? t.to_account_id?.slice(0,8)}</td>
                <td className="px-4 py-3 text-right font-semibold text-blue-600">{formatAmount(t.amount)}</td>
